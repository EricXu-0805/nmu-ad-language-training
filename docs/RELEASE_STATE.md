...
> **待上线（2026-08-27/28 本机完成，未部署）**：`e360896` 审查处置批次 +
> `b47f57d` 镜像扫描门禁转绿 + `26c5a80` 自我迭代（空断言、假告警、文档自相矛盾），
> **含迁移 `b6d4f8a2c917` → `c8e5a1f3b209`**，上线脚本在收据 228。
...
**有三个提交未部署**（见文首横幅）：`e360896` 审查处置批次（**含迁移
`b6d4f8a2c917` → `c8e5a1f3b209`**）、`b47f57d` 镜像扫描门禁转绿（只改 Dockerfile
与 SBOM，生产跑裸机、不跑这个镜像，对线上无影响）、`26c5a80` 自我迭代（两条空断言、
上线脚本会自己发一条假的站点告警、三处文档自相矛盾）。生产仍是 `ec2d1b5`。
