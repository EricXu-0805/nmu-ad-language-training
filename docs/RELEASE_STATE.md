...
## 2026-09-25 UTC 追加验收（应用仍为 `1ab262a`，未再次换版）

本轮前段新增隔离验收工具、回归测试与交接资料，未修改运行期应用、题库、评分、前端、数据库结构或现网配置，未重启生产应用；后续发现的独立备份校验器遗漏单列于本节末。现网业务应用仍是下方 `1ab262a`，不能把运维工具提交称为新的业务应用上线。

- **完整收尾与导出：** 新增 [独立 HTTP 演练](ISOLATED_WORKFLOW_REHEARSAL.md)，本机、无 Git／无默认数据目录的独立副本及同 VPS 隔离环境通过。最终 VPS 运行 61 次请求，从第 19 题开始，前 18 题明确跳过，后两题作答、暂停恢复、人工纠错、复核锁分、收尾完成及幂等导出通过。原 AI 结果保留；题号、19 条裁定和同一导出批次核验一致。420 个复制文件核对一致，其中 367 个运行期源码／内容文件与现网逐字节相同。最终脚本 SHA256 `c9d19e23a5ce1725f4fea25da9e421692e5916d82199cfad5beb1faf73952b4b`；八项新增回归通过。这是 TestClient 和合成音频，未验证公网认证后真实浏览器或现场设备。
- **录音容量：** [有界音频测试](CAPACITY_REHEARSAL.md) 在 VPS 实际写入 157,636,044 bytes，峰值 149,151,744 bytes，324 次哈希一致；使用独立文件和先写后轮转，不读取患者录音。新增工具 28 项、连同原容量工具 63 项测试通过。八周保守模型发现：30 人、每周一场、每场 78 回合、每回合两段 15 秒录音时，128 kbps 情景新增音频约 8.71 GiB，当前 14 份全量备份策略加写入峰值、额外副本与余量需约 162.39 GiB 起始可用空间；当前约 6.28 GiB 不足以覆盖该情景。这是明确假设的预算，不是当前磁盘已满或真实临床增长预测，未购买扩容、删录音或缩短保留期。
- **隔离与交接：** 临时进程禁止外网并屏蔽现网数据／配置目录，内存与 CPU 有上限。原始结果已拉回本机并核 SHA256 相同，服务器本轮临时目录已移除；清理后可用 6,742,609,920 bytes，应用、网关、既有网络服务及 HTTPS 健康均正常。补齐 [护理员交接表](CAREGIVER_REHEARSAL_HANDOFF.md)，纠正 README 仍称只能运行第 2 周的旧描述；没有把结构就绪解释为研究批准。

受控凭证：项目父目录 `审核报告_20260925/full-rehearsal/`。真实平板锁屏、外放回声、老人声音与弱网验收，实际规模下的存储方案和院方岗位／内容批准仍须各自完成；本轮工具不能替代这些事实。

容量口径补充：会议补充明确第 1 周为关系建立／基线，第 2–8 周共 7 次正式训练；原会议“每组 15 例共 30 人”为待定预实验规模。上述 30 人八批次是压力情景。按拟议 15 名训练者、7 次正式训练另算，128 kbps、相同备份与余量时需约 73.90 GiB 起始可用空间，仍超过当前余量；第 1 周录音尚未计入，也未据此购买扩容或变更保留期限。

### 后续恢复检查发现的备份校验器遗漏

将上述实际 API 生成的模拟数据交给原 `backup.sh` 时，复现 `export_metadata_invalid`：当前导出已有 13 张表，独立备份校验器仍只接受旧的 10 张表。本轮后续修复只更新独立校验器的合法历史表集合及对应验证，不修改业务应用、数据库、评分或导出内容。中间状态和已发布状态均绑定每个批次的精确表集合；路径、清单、哈希与音频闭包不放宽。新增完整链回归要求真实 API 导出经过原备份脚本、恢复校验和隔离启动，不能只以 HTTP 导出成功作为收尾。

该校验器必须同时更新 VPS 与 Mac 异地副本。实际安装版本、原文件回退副本、三处摘要及现场恢复结果以受控目录 `审核报告_20260925/full-rehearsal/backup-verifier-release/` 的回执和 VPS `/opt/nmu/last-backup-verifier.state` 核对；源码合并本身不证明已安装。业务应用身份继续按上方 `1ab262a` 记录，运维脚本身份单独登记。
