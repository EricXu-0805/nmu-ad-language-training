...
> **2026-09-07 当前阶段：维护隔离下的新版本已启动，发布收尾尚未完成。** 应用提交为 `5684a3a329b7189560942d6c2ec6a5f6444474bf`，库头为 `e2a6d8f0b419`。公网入口仍返回 **503**；新头备份的异地传输等待明确授权，六个 VPS 定时器与 Mac 异地调度均暂停。不得据此宣称已完成正常公网开放。
>
...
| 网关与拓扑 | systemd 裸机；Caddy `v2.11.4` 使用 Go `1.26.8` 和固定安全依赖重建，实际运行二进制已核对 | CI 对真实网关二进制及标准库的扫描覆盖 151 项；当前仍加载维护配置。未迁移到 Docker/Compose |
| 服务与入口 | 新应用私有 `/health` 为 200，匿名 `/patients` 为 401；公网入口为 503 | 私有启动成功不等于公网开放；匿名浏览器上线后检查尚未执行 |
| 数据库与保留对象 | 候选副本从 `d0c22a6dae2a` 迁移到 `e2a6d8f0b419`；旧完整应用树、旧数据和匹配旧头备份均保留 | 已核验副本迁移与原对象保留；本公开索引不登记业务对象数量、标识或私有审计锚点摘要 |
| 新头备份与异地 | 新头备份已在 VPS 完成完整校验；新头异地传输等待明确授权 | 匹配校验器、异地副本和审计前缀的最终闭环尚未完成，不能借用旧头异地副本宣布完成 |
| 定时任务 | 六个 VPS 定时器与 Mac 异地备份调度均暂停 | 待维护流程完成相应校验后恢复并回读；此刻不能把自动备份记为正常运行 |

详细维护、安装、源码核验和备份收据保存在项目父目录 `审核报告_20260906/` 的本轮受控审计记录中，不随公开仓库发布。当前结论是工程发布进行中；真实目标设备、内容签署和具名外部批准没有因这次 CI 或私有启动自动通过。
...
本表保留此前登记，表内“当前”“本次上线”等用语均指原记录日期。2026-09-07 的应用、库头、备份与定时任务状态以上方维护阶段表为准。
...
以上是收尾要求，不是本次全部完成的声明；2026-09-07 新头异地传输及调度恢复仍以上方阶段表为准。

## 本轮发布尚待完成

综合修复已合并、安装并完成维护隔离下的启动核验，详见 [修复进度与验收](audits/2026-09-06-fix-progress.md)。仍需完成获明确授权的新头异地传输、匹配校验器与审计前缀闭环、正常入口开放后的检查及定时任务恢复回读。公网仍为 503；不得将当前阶段写成发布全部完成，也不得沿用旧提交的 CI、旧头备份或目标设备记录代替本次剩余验收。
