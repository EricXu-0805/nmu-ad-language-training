...
**收据 249（零迁移；库头仍 `d0c22a6dae2a`；依赖锁/迁移图/校验器零变化）**：「回答正确但 AI 判 0 分」。
生产 196 次作答里 49 次含目标词的回答被云判分判成「重复」0 分（prompt 没有类型定义，qwen-plus
把「目标词说两遍」当重复；临床口径的重复是「重复一题答案」）。三道：确定式规则先定「整句就是那个词」
（含说两遍、带语气词）→ 正确、不问 LLM；老人把目标词当完整的词说出来而 LLM 初评是重复/偏题/未识别 → 改判部分正确待复核（仍记 LLM 辅助+原初评理由，出网记录不消失）；
prompt 写死六类定义与「双要素只判本轮目标词」。自动带练推进只看 contains_target，流程本就没走错；
改的是研究者屏上的 AI 初评与复核建议。上线用 `250-上线命令_判分口径.sh`。
