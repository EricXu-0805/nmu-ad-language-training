...
> **状态失效警告（2026-08-17，本机核验）**：下方“当前生产”最后一次现场核对仍是
> 2026-08-15。仓库 `main` 在本轮开始时已到 `836dcb6`，而本文仍写 `167273f`、
> `b3e7c5a9d214` 和“与 main 一致”；本轮又在本机准备了新的冻结研究行迁移
> `6f2a9c4d8e17`。本轮**没有登录生产机重新核验**，所以这些生产字段从现在起只能
> 当作“上一次已验证快照”，不能当作当前生产事实，也不能据此发布或回滚。恢复唯一
> 权威状态前，必须重新核对生产部署 SHA、数据库头、校验器摘要、服务状态和最新备份。
...
**未知，等待生产只读复核。** 本机仓库已经前进且有尚未提交的迁移与发布链修补；
在重新核对生产之前，不再声称 `main` 与部署树一致，也不把本机测试通过写成已上线。
