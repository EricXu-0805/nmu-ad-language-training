...
完整性设计(边界诚实标注,勿夸大):
  ...
  * 外部锚定(2026-08-07 起):异地拉取机每天用 scripts/audit_anchor_check.py 重算
    每份快照的链,并把 (count, tip) 记进异地追加账本——重算链+改锚点的"全权伪造"
    过不了异地已记的历史锚点(分叉/前缀改写/回退均判 TAMPER 并告警)。
  * 【已知残余】sha256 无密钥:锚定只保护"已被异地记账的历史";在两次拉取之间
    发生并当天被完整重写的增量,以及键控签名(HMAC)仍留待后续。本工具威胁模型是
