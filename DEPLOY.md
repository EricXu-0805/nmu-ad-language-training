...
> **现役拓扑（2026-09-07）**：仍为 systemd 裸机 `nmu` + `nmu-caddy`。当前应用、库头、备份和入口状态统一见 [部署状态索引](docs/RELEASE_STATE.md)；2026-09-06 的 `c2fd7fd` / `d0c22a6dae2a` 只描述修复前核验起点。本文 Docker Compose 部分是目标交付方案，尚不代表现役服务器已完成容器迁移。现役值班与恢复先查 [运维交接](docs/handover/运维交接_值班与故障处置.md)。
>
...
- `NMU_BEDSIDE_STALE_MINUTES=`（默认 15）：床旁槽「放置超时」阈值。研究者训练中直接关页签（不点收尾/中止）会留下占着床旁槽的活跃/暂停场，别人开新场被 409 挡。**遗弃只按控制台侧活动判**（场次 runtime 的活动时钟 / 床旁槽 live 写），**绝不含老人端被动心跳**——两设备部署里平板固定床旁一直心跳，研究者关的是笔记本控制台，把心跳算进来会让弃场永不让位。超过阈值无控制台活动的活跃场，下一位换人开场时被**安全暂停后让位**（原场仍可从恢复入口继续或收尾，一条记录不丢）；终态场（已收尾/中止）立即让位；两条让位都留审计（`bedside_slot_stale_takeover` / `bedside_slot_terminal_takeover`）。仍新鲜的活跃场维持原保护，切槽仍需上一场操作权。账号/受试者准入闸与握手层同口径（共享账号下弃场同样放行）。**取值：正整数=分钟阈值；0 或负数=关闭自动让位（永不 stale-takeover，只有终态才让位）；非数字落回 15。** 调大=更保守（更久才自动让位）。
...
- **单 worker**：失败限速器与第 1 周多轮对话的发声账本落账锁都在进程内。别给 uvicorn 加 `--workers>1`（限速会各自计数被削弱；轮次的「重数+落账」也会失去串行化，同一问位可能落成超轮次的行）；
