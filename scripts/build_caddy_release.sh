...
def build(root, args):
    ...
    with tempfile.TemporaryDirectory(prefix="nmu-caddy-build-") as temporary:
        ...
        # replacements. Every dependency stays governed by upstream go.mod/sum.
        ...
        staged.mkdir()
        binary = staged / "caddy"
        # Versioned install retains Caddy's real module version in the binary.
        # Building its unpacked directory would record (devel), hiding that
        # version from binary vulnerability scanners despite a correct receipt.
        run([str(go), "install", "-mod=readonly", "-trimpath", "-buildvcs=false",
             "-tags=" + ",".join(caddy["build_tags"]), "-ldflags=-s -w",
             caddy["module"] + "/cmd/caddy@" + caddy["version"]],
            env=env, cwd=work, capture=False)
        install_dir = work / "gopath/bin"
        if host != "linux-amd64":
            install_dir /= "linux_amd64"
        shutil.copyfile(install_dir / "caddy", binary)
        binary.chmod(0o755)
        run([str(go), "mod", "verify"], env=env, cwd=source, capture=False)
        ...
        if "\tpath\t" + caddy["module"] + "/cmd/caddy\n" not in build_info:
            raise ValueError("emitted binary has the wrong Caddy entrypoint")
        if "\tmod\t" + caddy["module"] + "\t" + caddy["version"] + "\t" not in build_info:
