...
if [ "$MODE" = "rollback" ]; then
  ...
  remote "$VENV_PY $APP_DIR/scripts/preflight_check.py --db $APP_DIR/data/app.db --backup-root $BACKUP_ROOT --lock $APP_DIR/requirements-deploy.lock.txt --os $preflight_url" || true
  exit 0
fi
...
echo "== 7. 验收 =="
if remote "$VENV_PY $APP_DIR/scripts/preflight_check.py --db $APP_DIR/data/app.db --backup-root $BACKUP_ROOT --lock $APP_DIR/requirements-deploy.lock.txt --os $preflight_url"; then
