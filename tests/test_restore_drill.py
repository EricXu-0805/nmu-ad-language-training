...
def _restored(tmp_path: Path, *, head: str = "f7c2e8a4d105",
...
def test_a_clean_restore_reports_head_and_row_counts(tmp_path):
    ...
    assert facts["alembic_head"] == "f7c2e8a4d105"
...
def test_a_foreign_key_violation_fails_the_drill(tmp_path):
    ...
    connection = sqlite3.connect(path)
    connection.execute("CREATE TABLE alembic_version (version_num TEXT)")
    connection.execute("INSERT INTO alembic_version VALUES ('f7c2e8a4d105')")
