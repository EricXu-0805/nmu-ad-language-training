...
export function ConfirmDialog({ open, title, body, confirmLabel = "确认", busy = false, confirmDisabled = false, onConfirm, onCancel }: {
    ...
    <div className="dialog-backdrop" onClick={cancelSafely}>
      <div ref={panelRef} className="dialog-panel fade-in" role="dialog" aria-modal="true"
        ...
        <div className="dialog-actions">
          ...
          <Button disabled={busy || confirmDisabled} variant="danger" onClick={onConfirm}>{confirmLabel}</Button>
        </div>
      </div>
    </div>
  );
