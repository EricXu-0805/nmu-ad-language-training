...
export function PatientEditDrawer({ patientId, sessionCount, onClose, onSaved }: {
  ...
  useEffect(() => {
    ...
    api.getPatient(patientId)
      .then((patient) => {
        ...
        setOriginal(loaded);
        setDraft(loaded);
        setBirthYearInput(loaded.birth_year == null ? "" : String(loaded.birth_year));
        setEducationInput(loaded.education_years == null ? "" : String(loaded.education_years));
  ...
  const save = async () => {
    ...
    if (birthYear.error || education.error) { setSaveError(birthYear.error ?? education.error); return; }
    const patch = buildProfilePatch(original, { ...draft, birth_year: birthYear.value, education_years: education.value }, newId, patientId);
            ...
            <div className="form-layout">
              <section className="form-section">
                ...
                <div className="form-grid">
                  ...
                  <Field error={birthYear.error} label="出生年份" hint="统计分析用的协变量；只填年份（如 1948）">
                    <TextInput inputMode="numeric"
                      value={birthYearInput} aria-invalid={Boolean(birthYear.error)}
                      onChange={(e) => { setSaveError(null); setBirthYearInput(e.target.value); }}
                  ...
                  <Field error={education.error} label="受教育年限" hint="按完成的正规教育年数填写，0–30 年">
                    <TextInput inputMode="numeric"
                      value={educationInput} aria-invalid={Boolean(education.error)}
                      onChange={(e) => { setSaveError(null); setEducationInput(e.target.value); }}
