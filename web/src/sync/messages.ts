...
function parseRapport(value: unknown, withType: boolean): RapportMsg | null {
  ...
  if (!row || !exactKeys(row,
    ...
    ["beat", "replyId", "utteranceId", "recSeq", "rawAudioId", "assentGate", "containsDirectIdentifier", "paused", "wseq"], withType)
  ...
  if (Object.hasOwn(row, "utteranceId")) {
    // 发声记录编号:正整数,指向服务端只追加账本的一行。
    if (!nonNegativeInteger(row.utteranceId) || row.utteranceId < 1) return null;
    result.utteranceId = row.utteranceId;
  }
